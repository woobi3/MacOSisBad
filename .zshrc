...
PROMPT="%{$fg[black]%}%K{27}%n%{$reset_color%}%{$bg[cyan]%}%F{27}%{$fg[black]%}%d%{$fg[cyan]%}%{$bg[black]%}"$'\n'"%{$bg[cyan]%}%{$fg[black]%}>%{$reset_color%}%{$fg[black]%} %k%f"
RPROMPT="%{$fg[black]%}%F{27}%{$fg[black]%}%K{27}%? | %t%k%f"
...
alias la='exa -la --time-style=long-iso --git --color-scale'  # all list
